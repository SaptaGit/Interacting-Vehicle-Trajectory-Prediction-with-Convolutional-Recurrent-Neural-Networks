    ...
    for vehicle in vehiclePositions:
        for idx, _ in enumerate(vehicle):
            if (idx+outputSize > len(vehicle)):
                break
            global folderCount
            folderPath = imageFolder + 'sample_' + str(folderCount)
            os.mkdir(folderPath)
            ...
            for sampleFrame in inputSampleFrames:
                ...
                for idx in range(2,frameLength):
                    if (localTime == sampleFrame[idx][4]):
                        absoluteXPixel = imageOriginX - int((sampleFrame[idx][0]-localOriginX)*latResolution)
                        absoluteYPixel = imageOriginY - int((sampleFrame[idx][1]-localOriginY)*lonResolution)
                        pixelLength = int(sampleFrame[idx][2]*lonResolution)
                        pixelWidth = int((sampleFrame[idx][3]*latResolution)/2)
            ...
            for outputSample in outputSampleFrames:
                ...
                relativeOutputTargetX = round((currentTargetX - localOriginX),3)
                relativeOutputTargetY = round((currentTargetY - localOriginY),3)
