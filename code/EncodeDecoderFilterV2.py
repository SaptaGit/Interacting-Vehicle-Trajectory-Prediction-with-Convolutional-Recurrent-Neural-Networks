...
trainFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/server/'
...
class CIFAR10Sequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        yTrajectory = np.array(sampleOutput).reshape(self.batch_size, 1, 2)

        newShiftedList = []

        for sdx in range(0,self.batch_size):
            newShiftedList.append([OccupancyImageWidth/2,OccupancyImageHeight/2])

        newShifted = np.array(newShiftedList).reshape(self.batch_size, 1, 2)

        # inputFinal = [inputTrajArray, outTrajShiftedArray]
        # outputFinal = [yTrajectory]

        inputFinal = [inputTrajArray, newShifted]
        outputFinal = [yTrajectory]
...
if __name__ == '__main__':
    ...
    decoder_dense7 = Dense(2048, activation='relu')
    decoder_output2 = decoder_dense7(decoder_output2)

    decoder_dense6 = Dense(1024, activation='relu')
    decoder_output2 = decoder_dense6(decoder_output2)

    decoder_dense5 = Dense(512, activation='relu')
    decoder_output2 = decoder_dense5(decoder_output2)

    decoder_dense4 = Dense(256, activation='relu')
    decoder_output2 = decoder_dense4(decoder_output2)

    #act = LeakyReLU()
    decoder_dense3 = Dense(128, activation='relu')
    decoder_output2 = decoder_dense3(decoder_output2)

    #act1 = LeakyReLU()
    decoder_dense2 = Dense(64, activation='relu')
    decoder_output2 = decoder_dense2(decoder_output2)
    
    decoder_dense1 = Dense(16, activation='relu')
    decoder_output2 = decoder_dense1(decoder_output2)
    ...
    trainModel.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=200, verbose=1, callbacks=callbacks_list)
    ...
    #decoder_outputs, state_h, state_c = decoder_lstm(decoderNorm, initial_state=decoder_states_inputs)
    ...
    #decoder_states = [decoder_h1, decoder_c1]
    decoder_outputs = decoder_dense7(decoder_outputs)
    decoder_outputs = decoder_dense6(decoder_outputs)
    decoder_outputs = decoder_dense5(decoder_outputs)
    ...
    f = open('/home/saptarshi/PythonCode/AdvanceLSTM/server/0/output.txt', 'r')
    #f = open('/home/saptarshi/PythonCode/AdvanceLSTM/server/0/output.txt', 'r')
    outputLines = f.read().splitlines()
    ...
    for t in range(30):
        # predict next char
        yhat = decoder_model.predict([target_seq] + state)
        # store prediction
    ...
    originalLonError = FrameLonError(outputTrajArray,resultOriginal)
