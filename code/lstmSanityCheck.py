...
class CIFAR10Sequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        X = np.array(sampleInput).reshape(self.batch_size,imageHeight,imageWidth, temporal)
        y = np.array(sampleOutput).reshape((self.batch_size,60))
...
if __name__ == '__main__':
    ...
    model = Sequential()
    # define CNN model
    #model.add(Conv2D(4, (5,5), activation='relu', padding='same'), input_shape=(temporal,imageHeight,imageWidth,channel))
    model.add(Conv2D(4, (5,5), activation='relu', padding='same', input_shape=(imageHeight,imageWidth,30)))
