...
imageFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/SuperDuperhigh/'
...
def generateOccupancyGrid(vehiclePositions):
    ...
    # LaneChangeVehicleList = totalVehicleList[:50]

    # print('Vehicle IDs to be process are : ' + str(len(LaneChangeVehicleList)))


    # idList = vehiclePositions[:][0][0]
    # for rand in range(0,20):
    #     randomIndexList.append(random.randint(1,idList))

    for vehicle in vehiclePositions:
        currentVehicleID = vehicle[0][0][0]
        if(currentVehicleID not in LaneChangeVehicleList):
            continue
        # if(currentVehicleID not in LaneChangeVehicleList) or (currentVehicleID not in randomIndexList):
        #     continue
