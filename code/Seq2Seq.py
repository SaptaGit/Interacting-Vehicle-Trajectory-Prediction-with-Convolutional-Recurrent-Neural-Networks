...
trainFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/jointsmall/'
...
class LossHistory(callbacks.Callback):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        print('Batch End')
        print(self.var_y_true.value)
        self.targets.append(K.eval(self.var_y_true))
        self.outputs.append(K.eval(self.var_y_pred))
        y_true = tf.Print(self.var_y_true, [self.var_y_true], "True:", summarize=100)
        y_pred = tf.Print(self.var_y_pred, [self.var_y_pred], "Predicted:", summarize=100)
...
if __name__ == '__main__':
    ...
    opt = RMSprop()
    fetches = [tf.assign(loss_history.var_y_true, trainModel.targets , validate_shape=False),
           tf.assign(loss_history.var_y_pred, trainModel.outputs[0], validate_shape=False)]
    ...
    trainModel.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=400, verbose=1, callbacks=callbacks_list)
    #trainModel.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=400, verbose=1)


    #read input data

    f = open('/home/saptarshi/PythonCode/AdvanceLSTM/Joint/15/output.txt', 'r')
    outputLines = f.read().splitlines()
    inputTestTrajstr = outputLines[:-2]
    inputTestTraj = []
    for eachPoint in inputTestTrajstr:
        testX = float(eachPoint.split(',')[0])/float(OccupancyImageWidth)
        testY = float(eachPoint.split(',')[1])/float(OccupancyImageHeight)
