...
testTrajFilePath = '/home/saptarshi/Downloads/highD-dataset-v1.0/data/37_tracks.csv'
metaCsvFile = '/home/saptarshi/Downloads/highD-dataset-v1.0/data/37_tracksMeta.csv'
# Load the pre trained model
modelFilePath = '/home/saptarshi/PythonCode/AdvanceLSTMServer/TrainedModels/HighDLane200Epoch.h5'
#otherModelPath = '/home/saptarshi/PythonCode/AdvanceLSTMServer/TrainedModels/HighDV1.h5'
...
imageFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/PlaceHolderForPrediction1/'
...
# Set the Folder for gloabl frame predicition results
resultFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/Results/VideoTest1/'
# Set the Folder for local frame predicition results
localResultFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/Results/LocalVideo/'
# Set the Folder for Input OGM Frames
inputOGMFolder = '/home/saptarshi/PythonCode/AdvanceLSTM/Results/InputOgmVideo/'
...
LaneChangeVehicleList = [239.0]# 239.0 done
...
#otherModel = load_model(otherModelPath, custom_objects={'euclidean_distance_loss': euclidean_distance_loss, 'euclidean_distance_loss' : euclidean_distance_loss})
...
def CreateVehicleAndFrameDict(loadFileName, metaFileName):
    ...
    return dictionaryByFrames,dictionaryByVehicles,metaDict
...
# Perfrom the predicition for all the cars in the scene and plot them in the global frame and save in the specified folder.
def PredictForTargetCars(inputFileName, metaCsvFile):
    ...
    imageCount = 0
    outputCount = 0
    dictByFrames,dictByVehicles,metaDict = CreateVehicleAndFrameDict(inputFileName, metaCsvFile)
    finalVehicleKeys = list(dictByVehicles.keys())
    finalVehicleKeys.sort(key=float)
    finalFrameKeys = list(dictByFrames.keys())
    finalFrameKeys.sort(key=float)
    ...
    for keyVal in finalVehicleKeys:
        ...
        for vehicleVal in vehicleData:
            ...
            frameSpecificList = []
            frameSpecificList.append([VehicleID, FrameID])
            frameSpecificList.append([VehicleX, VehicleY, VehicleLength, VehicleWidth, FrameID, VehicleSpeed])
            for frameVal in frameData:
                frameSpecificVehicleID = frameVal[0]
                ...
                if (frameSpecificVehicleID != VehicleID) and (FrameID==currentOtherFrame) and (currentDirivngDirection==otherDirivngDirection):
                    ...
                    currentSpeed = frameVal[13]
                    frameSpecificList.append([currentX, currentY, currentLength, currentWidth, currentOtherFrame, frameSpecificVehicleID, currentSpeed])
            vehicleSpecificList.append(frameSpecificList)
        relativePositionList.append(vehicleSpecificList)
    ...
    for vehicle in relativePositionList:
        ...
        for idx, _ in enumerate(vehicle):
            ...
            inputSampleFrames = vehicle[idx]
            primaryCarId = inputSampleFrames[0][0]
            currentTargetX, currentTargetY, length, width, localFrame, targetSpeed = inputSampleFrames[1]
            carsInCurrentFrame = []
            carsInCurrentFrame.append(primaryCarId)
            if primaryCarId in trackerDict:
                trackerDict[primaryCarId].append([currentTargetX, currentTargetY, length, width, localFrame, targetSpeed])
            else:
                trackerDict[primaryCarId] = [[currentTargetX, currentTargetY, length, width, localFrame, targetSpeed]]

            frameLength = len(inputSampleFrames)
            for jdx in range(2,frameLength):
                currentOtherX, currentOtherY, otherLength, otherWidth, otherLocalFrame, otherCarId, otherSpeed = inputSampleFrames[jdx]
                if (localFrame == otherLocalFrame):
                    carsInCurrentFrame.append(otherCarId)
                    if otherCarId in trackerDict:
                        trackerDict[otherCarId].append(inputSampleFrames[jdx])
                    else:
            ...
            if ( len(trackerDict[primaryCarId]) == outputSize):
                ...
                for ldx,sampleTargetFrame in enumerate(targetCar[0:inputSize]):
                    ...
                    # For input OGM video
                    currentOGMInput[int(relativeTargetY):int(relativeTargetY+pixelLengthTarget),int(relativeTargetX-pixelWidthTarget):int(relativeTargetX+pixelWidthTarget),1] = 0
                    currentOGMInput[int(relativeTargetY):int(relativeTargetY+pixelLengthTarget),int(relativeTargetX-pixelWidthTarget):int(relativeTargetX+pixelWidthTarget),2] = 0
                    for trackerKey in trackerDict:
                        ...
                        if corespndingOtherCarIndex >= 0 and trackerKey!= primaryCarId and localTime == trackerDict[trackerKey][corespndingOtherCarIndex][4]:
                            ...
                            absXDistance = abs(currentOtherXCheck-primaryLocalOriginX)*latResolution
                            absYDistance = abs(currentOtherYCheck-primaryLocalOriginY)*lonResolution
                            if((absXDistance>=(occupancyMapWidth/2)) or (absYDistance>=(occupancyMapHeight/2))):
                                continue
                            ...
                            currentFrame[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth)] = 128
                            # For input OGM video
                            currentOGMInput[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth),0] = 0
                            currentOGMInput[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth),1] = 0

                    im = Image.fromarray(currentOGMInput)
                    imageCount = imageCount+1
                    imFileName = imageFolder + targetCarFolder + '/' + str(imageCount) + imageFileType
                    primaryImageFiles[0,ldx,:,:,0] = currentFrame/255
                    #primaryImageFiles[0,ldx,:,:,1] = velocityFrame/100
                ...
                for t in range(1,predTemporal+1):
                    ...
                    if predictedPoseY > 254 : #OccupancyImageHeight/2:
                        print('In the wrong Update')
                        predictedPoseY = (OccupancyImageHeight/2) - 2 ##- 2
                    ...
                    if not targetCarPredictedPoses:
                        ...
                        newPredX = lastPredX + 3*shiftX #weired fix  - shiftX
                    ...
                    for vdx in range(0,temporal):
                        ...
                        for trackerKey in trackerDict:
                            ...
                            if corespndingOtherCarIndex >= 0 and trackerKey!= primaryCarId and localTime == trackerDict[trackerKey][corespndingOtherCarIndex][4]:
                                ...
                                absoluteXPixel = imageOriginX + int((currentOtherXCheck-primaryLocalOriginX)*latResolution)  #Was + before
                                absoluteYPixel = imageOriginY - int((currentOtherYCheck-primaryLocalOriginY)*lonResolution)
                                # otherCarsPixelDict[trackerKey] = [[absoluteXPixel,absoluteYPixel]]
                                # else:
                                ...
                                #     pxShiftX = (currentOtherXCheckOld - currentOtherXCheck)*latResolution
                                #     pxShiftY = (currentOtherYCheckOld - currentOtherYCheck)*lonResolution
                                ...
                                #     absoluteXPixel = lastXPixel + pxShiftY
                ...
                for outputTargetSample in outputTargetSampleFrames:
                    currentTargetX, currentTargetY, length, width, localTime, unusedSpeed = outputTargetSample
                    relativeOutputTargetX = imageOriginX + (int((currentTargetX - primaryLocalOriginXOriginal)*latResolution))*2
                ...
                pts = np.array(targetCarGroundTruthPoseList[:40], np.int32)
                pts = pts.reshape((-1,1,2))
                cv2.polylines(opencvVisulaizeImage,[pts],False,(0,255,0),1)
                textPose = pts[0,0]
                font = cv2.FONT_HERSHEY_SIMPLEX
                ...
                pts = pts.reshape((-1,1,2))
                cv2.polylines(opencvVisulaizeImage,[pts],False,(255,0,0),1)
                textPose = pts[0,0]
                font = cv2.FONT_HERSHEY_SIMPLEX

                # Draw lane markings
                cv2.line(opencvVisulaizeImage,(52,0),(52,510),(0,0,255),1)
                cv2.line(opencvVisulaizeImage,(62,0),(62,510),(0,0,255),1)
                cv2.line(opencvVisulaizeImage,(72,0),(72,510),(0,0,255),1)
                cv2.line(opencvVisulaizeImage,(82,0),(82,510),(0,0,255),1)
                cv2.line(opencvVisulaizeImage,(92,0),(92,510),(0,0,255),1)
                cv2.line(opencvVisulaizeImage,(102,0),(102,510),(0,0,0),1)
    ...
    # Perfrom the predicition for all the cars in the scene and plot them in the global frame and save in the specified folder.
def PredictForAllCarsNew(inputFileName, metaCsvFile):
    ...
    for vehicle in relativePositionList:
        ...
        for idx, _ in enumerate(vehicle):
            ...
            if ( len(trackerDict[primaryCarId]) == outputSize):
                ...
                #globalYOffset = int(primaryLocalOriginYOriginal * lonResolution)
                ...
                for ldx,sampleTargetFrame in enumerate(targetCar[0:inputSize]):
                    ...
                    currentFrame = np.zeros((OccupancyImageHeight,OccupancyImageWidth), dtype=np.uint8)
                    currentFrame.fill(255)
                    currentOGMInput = np.zeros((OccupancyImageHeight,OccupancyImageWidth,3), dtype=np.uint8)
                    currentOGMInput.fill(255)
                    # velocityFrame = np.zeros((OccupancyImageHeight,OccupancyImageWidth), dtype=np.float)
                    # velocityFrame.fill(0)
                    currentTargetX, currentTargetY, length, width, localTime, targetSpeed = targetCar[ldx]
                    # Check if the target car in within the frame
                    ...
                    im = Image.fromarray(currentOGMInput)
                    imageCount = imageCount+1
                    imFileName = inputOGMFolder + '/' + str(imageCount) + imageFileType
                    primaryImageFiles[0,ldx,:,:,0] = currentFrame/255
                    #primaryImageFiles[0,ldx,:,:,1] = velocityFrame/100
                    im.save(imFileName)

                #opencvVisulaizeImage = cv2.cvtColor(currentFrame, cv2.COLOR_GRAY2BGR)
                opencvVisulaizeImage = currentOGMInput
                ...
                for t in range(1,predTemporal+1):
                    ...
                    if not targetCarPredictedPoses:
                        ...
                        lastPredX = targetCarPredictedPoses[-1][0]
                        lastPredY = targetCarPredictedPoses[-1][1]
                        newPredX = lastPredX +  shiftX*5 # - for vehicle 281 - for 469 + #Something weird check   - shiftX
                        newPredY = lastPredY - shiftY
                        targetCarPredictedPoses.append([newPredX,newPredY])
                    ...
                    # Update the origins
                    primaryLocalOriginX = primaryLocalOriginX + (shiftX*(1/latResolution)*20) # *20 for better x shift
                    primaryLocalOriginY = primaryLocalOriginY + (shiftY*(1/lonResolution))
                ...
                outputTargetSampleFrames = targetCar[inputSize:outputSize]
                targetCarGroundTruthPoseList = []
                targetCarGroundTruthGlobalPoseList = []
                for outputTargetSample in outputTargetSampleFrames:
                    currentTargetX, currentTargetY, length, width, localTime, unusedSpeed = outputTargetSample
                    relativeOutputTargetX = imageOriginX +( int((currentTargetX - primaryLocalOriginXOriginal)*latResolution))*3    # + for 469
                    relativeOutputTargetY = imageOriginY - int((currentTargetY - primaryLocalOriginYOriginal)*lonResolution)
                    globalOutputTargetX =  int((currentTargetX-dueToWidthOffset)*globalWidthResolution)    # + for 469
                    globalOutputTargetY =  int((currentTargetY+dueToNegetiveValuesOffset)*globalHeightResolution)
                    targetCarGroundTruthPoseList.append([relativeOutputTargetX,relativeOutputTargetY])
                    targetCarGroundTruthGlobalPoseList.append([globalOutputTargetX,globalOutputTargetY])

                # Plot the Globalground Truth
                pts = np.array(targetCarGroundTruthGlobalPoseList[:30], np.int32)
                pts = pts.reshape((-1,1,2))
                cv2.polylines(globalImage,[pts],False,(0,255,0),2)
                #cv2.polylines(opencvVisulaizeImage,[pts],False,(0,255,0),1)
                textPose = pts[0,0]
                font = cv2.FONT_HERSHEY_SIMPLEX
                #cv2.putText(opencvVisulaizeImage, str(int(primaryCarId)), (textPose[0],textPose[1]), font, 0.4,(0,0,255),1,cv2.LINE_AA)
                cv2.putText(globalImage, str(int(primaryCarId)), (textPose[0],textPose[1]), font, 0.4,(0,0,255),1,cv2.LINE_AA)

                # Draw Target car on GlobalImage
                globalTargetWidth = (width * globalWidthResolution)/2
                globalTargetLength = length * globalHeightResolution
                globalImage[int(textPose[1]):int(textPose[1]+globalTargetLength),int(textPose[0]-globalTargetWidth):int(textPose[0]+globalTargetWidth)] = 0

                # Plot the Predicted 
                # Update predicted poses for global settings
                # for xdx,_ in enumerate(targetCarPredictedPoses):
                #     targetCarPredictedPoses[xdx][1] = targetCarPredictedPoses[xdx][1] + globalYOffset

                # Plot the local predicted
                pts = np.array(targetCarPredictedPoses, np.int32)
                pts = pts.reshape((-1,1,2))
                #cv2.polylines(opencvVisulaizeImage,[pts],False,(0,0,255),1)
                textPose = pts[0,0]
                font = cv2.FONT_HERSHEY_SIMPLEX

                # Plot the local ground Truth
                pts = np.array(targetCarGroundTruthPoseList, np.int32)
                pts = pts.reshape((-1,1,2))
                cv2.polylines(opencvVisulaizeImage,[pts],False,(0,255,0),1)
                #cv2.polylines(opencvVisulaizeImage,[pts],False,(0,255,0),1)
                ...
                for otherTrackedEligibleCarKey in trackerDict:
                    ...
                    if otherTrackedEligibleCarKey != primaryCarId and len(trackerDict[otherTrackedEligibleCarKey]) == outputSize:
                        ...
                        #globalYOffsetOther = otherTargetLocalOriginYOriginal * lonResolution
                        ...
                        for ndx,sampleOtherFrame in enumerate(otherCarDataForPrediction[0:inputSize]):
                            ...
                            for forOtherTrackerKey in trackerDict:
                                ...
                                if corespndingOtherCarIndex >= 0 and forOtherTrackerKey!= currentKey and localTime == trackerDict[forOtherTrackerKey][corespndingOtherCarIndex][4]:
                                    ...
                                    currentFrame[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth)] = 0
                                    #otherVelocityFrame[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth)] = otherSpeed
                        ...
                        for t in range(1,predTemporal+1):
                            ...
                            predictedOtherPoses = model.predict(otherImageFiles)   # model.predict(otherImageFiles)

                            #####print( 'Other pose ' + str(predictedOtherPoses[0][0]) + '\n')
                            ...
                            if not otherCarPredictedPose:
                                ...
                                newPredX = predictedPoseX  # lastPredX + shiftX #something weird check - shiftX
                                newPredY = lastPredY - shiftY
                            ...
                            otherTargetLocalOriginY = otherTargetLocalOriginY + (shiftY*(1/lonResolution))
                            ...
                            for qdx in range(0,temporal):
                                ...
                                # imageCount = imageCount+1
                                # imFileName = otherFolderPath + '/' + str(imageCount) + imageFileType
                                otherImageFiles[0,qdx,:,:,0] = currentFrame/255
                                #primaryImageFiles[0,ldx,:,:,1] = velocityFrame/100
                                # im.save(imFileName)
                        ...
                        otherTargetCarGroundTruthGlobalPoseList = []
                        for outputTargetSample in outputTargetSampleFrames:
                            ...
                            otherTargetCarGroundTruthGlobalPoseList.append([globalOutputTargetX,globalOutputTargetY])

                        # Plot the global ground Truth
                        pts = np.array(otherTargetCarGroundTruthGlobalPoseList[:30], np.int32)
                        pts = pts.reshape((-1,1,2))
                        #cv2.polylines(opencvVisulaizeImage,[pts],False,(0,255,0),1)
                        cv2.polylines(globalImage,[pts],False,(0,255,0),2)
                        textPose = pts[0,0]
                        ...
                        cv2.putText(globalImage, str(int(otherTrackedEligibleCarKey)), (textPose[0],textPose[1]), font, 0.4,(0,0,255),1,cv2.LINE_AA)

                        # Draw Other Target car on GlobalImage 
                        otherGlobalLength = length*globalHeightResolution
                        otherGlobalWidth = (width*globalWidthResolution)/2
                        globalImage[int(textPose[1]):int(textPose[1]+otherGlobalLength),int(textPose[0]-otherGlobalWidth):int(textPose[0]+otherGlobalWidth)] = 128
                        ...
                        # Plot local the Predicted 
                        # # Update for gloabl settings
                        # for ydx,_ in enumerate(otherCarPredictedPose):
                        #     otherCarPredictedPose[ydx][1] = otherCarPredictedPose[ydx][1] + globalYOffsetOther

                        pts = np.array(otherCarPredictedPose, np.int32)
                        pts = pts.reshape((-1,1,2))
                        #cv2.polylines(globalImage,[pts],False,(0,0,255),2)
                        #cv2.polylines(opencvVisulaizeImage,[pts],False,(0,0,255),1)
                        textPose = pts[0,0]
                        font = cv2.FONT_HERSHEY_SIMPLEX

                        # Append the all other car predicted poses for lates plot

                        otherAllCarPredictedPoses.append(otherCarPredictedPose)
                ...
                # Draw lane markings
                # cv2.line(opencvVisulaizeImage,(35,0),(35,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(46,0),(46,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(57,0),(57,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(67,0),(67,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(77,0),(77,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(89,0),(89,510),(255,0,0),1)
                # cv2.line(opencvVisulaizeImage,(99,0),(99,510),(255,0,0),1)

                rotated = np.rot90(globalImage)
                # rotated = np.rot90(rotated)
                # rotated = np.rot90(rotated)

                #targetCarPredictedPoses and otherAllCarPredictedPoses

                for plotIdx in range(0,len(targetCarPredictedPoses)):
                    cv2.circle(opencvVisulaizeImage, (int(targetCarPredictedPoses[plotIdx][0]),int(targetCarPredictedPoses[plotIdx][1])), 1, (0, 0, 255), -1)
                    for eachOtherPred in otherAllCarPredictedPoses:
                        cv2.circle(opencvVisulaizeImage, (int(eachOtherPred[plotIdx][0]),int(eachOtherPred[plotIdx][1])), 1, (0, 0, 255), -1)

                    cv2.imwrite(localResultFolder + str(plotIdx) + '.png', opencvVisulaizeImage)
                
                print('All cars pose plotted')
                sys.exit()                    


                cv2.imwrite(resultFolder + str(globalLonErrorCount) + '.png', rotated)

                cv2.imwrite(localResultFolder + str(globalLonErrorCount) + '.png', opencvVisulaizeImage)
...
if __name__ == '__main__':
    ...
    #PredictForTargetCars(testTrajFilePath, metaCsvFile)
    PredictForAllCarsNew(testTrajFilePath, metaCsvFile)
