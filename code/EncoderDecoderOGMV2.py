...
trainFolder =  '/home/saptarshi/PythonCode/AdvanceLSTM/maptest/'
    ...
    def __getitem__(self, idx):
        ...
        for currentSample in currentBatchfileList:
            ...
            for ldx,imageFile in enumerate(sortedImageFiles):
                OGMMap = cv2.imread(self.path + currentSample + '/' +  imageFile, cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
   ...
   # Basic LSTM structure with Conv2D
    ...
    output = TimeDistributed(MaxPooling2D(pool_size=(3,3)))(output)
    output = TimeDistributed(BatchNormalization())(output)
    output, state_c2, state_h2 = ConvLSTM2D(
        filters=4,
        kernel_size=(5, 5),
        activation='relu',
        padding='same',
        return_sequences=True,
        return_state=True)(output)
    output = TimeDistributed(MaxPooling2D(pool_size=(5,5)))(output)
    output = TimeDistributed(BatchNormalization())(output)
    output, state_c3, state_h3 = ConvLSTM2D(
        filters=8,
        kernel_size=(7, 7),
        activation='relu',
        padding='same',
        return_sequences=True,
        return_state=True)(output)
    output = TimeDistributed(MaxPooling2D(pool_size=(7,7)))(output)
    output = TimeDistributed(BatchNormalization())(output)

    #model.add(Flatten())
    output = TimeDistributed(Dense(256))(output)
    output = TimeDistributed(LeakyReLU())(output)
    output = TimeDistributed(Dense(128))(output)
    output = TimeDistributed(LeakyReLU())(output)
    output = TimeDistributed(Dense(64))(output)
    output = TimeDistributed(LeakyReLU())(output)
    output = TimeDistributed(Dense(32))(output)
    output = TimeDistributed(LeakyReLU())(output)
    output = Flatten()(output)

    stateConcatenated2 = concatenate([state_c2, state_h2])
    flatenConcatenate2 = Flatten()(stateConcatenated2)

    stateConcatenated3 = concatenate([state_c3, state_h3])
    flatenConcatenate3 = Flatten()(stateConcatenated3)

    outputFinal = concatenate([output, flatenConcatenate2, flatenConcatenate3])

    outputFinal = Dense(128)(outputFinal)
    outputFinal = LeakyReLU()(outputFinal)
    outputFinal = Dense(64)(outputFinal)
    outputFinal = LeakyReLU()(outputFinal)
    outputFinal = Dense(32)(outputFinal)
    outputFinal = LeakyReLU()(outputFinal)
    outputFinal = Dense(16)(outputFinal)
    outputFinal = LeakyReLU()(outputFinal)
    outputFinal = Dense(2, activation='linear')(outputFinal)
    ...
    model = Model(input=[ogmInput], output=[outputFinal])
    ...
    #history = model.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=60, verbose=1)
    history = model.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=10, verbose=1, callbacks=callbacks_list)
    model.save('OGMConvLSTMConcat.h5')
