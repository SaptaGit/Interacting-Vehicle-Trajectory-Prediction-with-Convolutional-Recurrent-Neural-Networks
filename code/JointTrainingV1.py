...
class CIFAR10Sequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for currentSample in currentBatchfileList:
            ...
            tempClassVal = [float(classLabelStr.split(',')[0]), float(classLabelStr.split(',')[1])]
...
if __name__ == '__main__':
    ...
    ManeuverClass = Dense(2, activation='softmax', name='ManeuverClass')(Dense2)
    ...
    multipleSoftmax = RepeatVector(temporal)(ManeuverClass)
    concatinatedSoftmax =  concatenate([flatten1, multipleSoftmax])

    # Define Dense Part One
    Dense1 = TimeDistributed(Dense(512, activation='relu'))(concatinatedSoftmax)
    ...
    model = Model(inputs=ogmInput, outputs=[ManeuverClass, trajectory])
    losses = {
    ...
    history = model.fit_generator(trainGen, steps_per_epoch=stepsPerEpoch, epochs=140, verbose=1)
