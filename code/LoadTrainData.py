...
LaneChangeVehicleList = [87.0,44.0,54.0,50.0,41.0,31.0,115.0,32.0,45.0,121.0,60.0,144.0]
    ...
    for vehicle in vehiclePositions:
        ...
        for idx, _ in enumerate(vehicle):
            ...
            for sampleFrame in inputSampleFrames:
                ...
                forColor = np.zeros((OccupancyImageHeight,OccupancyImageWidth), dtype=np.uint8)
                forColor.fill(0)
                ...
                velocityFrame[int(relativeTargetY):int(relativeTargetY+pixelLength),int(relativeTargetX-pixelWidth):int(relativeTargetX+pixelWidth)] = int(targetSpeed)
                ...
                for jdx in range(2,frameLength):
                    ...
                    if (localTime == otherLocalTime):
                        ...
                        velocityFrame[int(absoluteYPixel):int(absoluteYPixel+pixelLength),int(absoluteXPixel-pixelWidth):int(absoluteXPixel+pixelWidth)] = int(otherSpeed)
                concatinatedFrameArray[:,:,0] = forColor
                concatinatedFrameArray[:,:,1] = velocityFrame
                concatinatedFrameArray[:,:,2] = currentFrame
